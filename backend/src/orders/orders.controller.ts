...
export class OrdersController {
    ...
    splitTable(
        ...
        @Body() body: { targetTableNumber: string; items: { id: number; quantity: number }[]; newOrderNumber?: string },
    ): Promise<Order> {
        return this.ordersService.splitTable(orderNumber, body.targetTableNumber, body.items, body.newOrderNumber);
    }

    @Post(':id/pay-items')
    payItems(
        @Param('id') id: string,
        @Body() body: { items: { id: number; quantity: number }[]; paymentMethod: string },
    ): Promise<Order> {
    ...
    mergeTables(
        ...
        return this.ordersService.mergeTables(body.fromTableNumber, body.toTableNumber);
    }
}
